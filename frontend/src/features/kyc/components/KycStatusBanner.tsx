...
const isDlExpired = (dateStr?: string | null): boolean => {
  if (!dateStr) return false;
  return new Date(dateStr) < new Date();
};

export const KycStatusBanner = ({ kycStatus, dlExpiryDate, compact = false }: KycStatusBannerProps) => {
  const licenseExpired = isDlExpired(dlExpiryDate);

  // Verified + expired DL → show warning, not null
  if (kycStatus === 'verified' && licenseExpired) {
    if (compact) {
      return (
        <div className="flex items-center justify-between gap-3 text-sm text-red-700 bg-red-50 border border-red-200 px-3 py-2 rounded-lg">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="font-medium truncate">License Expired</span>
          </div>
          <Link
            to="/portal/kyc"
      ...
      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h3 className="text-sm font-semibold text-red-800">Driving License Expired</h3>
            <p className="mt-1 text-sm text-red-700 mb-4">
              Your license expired on {dlExpiryDate}. Please upload a renewed license to continue booking vehicles.
            </p>
            <Link
              to="/portal/kyc"
