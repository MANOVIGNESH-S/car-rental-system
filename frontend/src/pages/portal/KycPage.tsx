...
import { ShieldCheck, FileText, Camera, CheckCircle2, AlertTriangle, Loader2, X } from 'lucide-react';
...
const RequirementsBox = () => (
  <div className="bg-gray-50 border border-gray-100 rounded-xl p-5 mt-6">
    <h4 className="text-sm font-semibold text-gray-900 mb-3">Requirements for Successful Verification</h4>
...
const FileInputCard = ({ id, label, description, icon: Icon, file, previewUrl, onChange, disabled }: FileInputProps) => {
  ...
  const baseClasses = "relative group border-2 border-dashed rounded-2xl p-6 text-center cursor-pointer transition-colors flex flex-col items-center justify-center min-h-[160px]";
  const stateClasses = file 
    ? "border-blue-400 bg-blue-50" 
    : "border-gray-300 hover:border-blue-400 hover:bg-gray-50 bg-white";
  const disabledClasses = disabled ? "opacity-60 cursor-not-allowed hover:border-gray-300 hover:bg-white" : "";
...
const KycPage = () => {
  const { kycStatus, isLoadingStatus, isUploading, uploadError, uploadSuccess, upload } = useKYC();
  ...
  // Default to 'pending' if no status exists yet
  const currentStatus = kycStatus?.kyc_status || 'pending'; 
  
  const isVerified = currentStatus === 'verified';
  
  // Only treat as processing if it's explicitly under review OR if they just successfully uploaded in this session
  const isProcessing = currentStatus === 'needs_review' || uploadSuccess;
  
  // They can upload if they are NOT verified and NOT currently processing
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center justify-between gap-4 mb-5 pb-5 border-b border-gray-100">
            ...
            <Badge variant={getKycStatusVariant(currentStatus)}>
              {currentStatus === 'pending' ? 'Not Verified' : currentStatus.replace('_', ' ')}
            </Badge>
          </div>
          ...
          {isProcessing && (
            <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl text-sm">
              ...
              <div>
                ...
                <p className="mt-0.5 text-amber-700">Your documents are being processed. This usually takes 1-2 business days. We'll notify you once complete.</p>
              </div>
            </div>
          )}
          ...
          {currentStatus === 'pending' && !isProcessing && (
             <div className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-xl p-4">
               You need to verify your identity to start booking vehicles. Please upload your documents below.
             </div>
          )}
        </div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
            ...
            <div className="mt-6 space-y-4">
              ...
              {uploadSuccess && (
                <div className="rounded-lg bg-green-50 border border-green-200 p-4 flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-green-800">Documents uploaded successfully!</p>
                    <p className="text-sm text-green-700 mt-0.5">Verification is in progress. We will update your status shortly.</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="mt-8">
              <button 
                ...
                className="w-full flex items-center justify-center gap-2.5 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white text-base font-semibold rounded-xl transition-colors disabled:opacity-60 disabled:cursor-not-allowed shadow-sm"
              >
                {isUploading ? (
                  <>
